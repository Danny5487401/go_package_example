...
func main() {
	...
	InsertDataToMongo(collection)

}

func InsertDataToMongo(collection *mongo.Collection) {
	// 构建数据
	//lr := &model.LogRecord{
	//	JobName: "Python",
	//	Command: "echo 4",
	...
	fourteenDayBefore := 24 * 15
	fourteenDayBeforeStr := fmt.Sprintf("-%dh", fourteenDayBefore)
	d, _ := time.ParseDuration(fourteenDayBeforeStr)
	sec := time.Now().Add(d).Format("20060102")
	secDate, _ := strconv.Atoi(sec)

	insertData := &model.TestLog{
		Uid:  1234,
		Day:  int64(secDate),
		Coin: 300,
	}
	////插入某一条数据
	iResult, err := collection.InsertOne(context.Background(), insertData)
	if err != nil {
		fmt.Println("错误", err)
		return
	}
...
func UpdateData(collection *mongo.Collection) {
	...
	//update := bson.M{"$set": model.UpdateByJobName{Command: "byModel", Content: "models"}}
...
func DeleteData(collection *mongo.Collection) {
	// 删除数据
	...
	//}
	//fmt.Println(uResult3.DeletedCount)

	var uResult3 *mongo.DeleteResult
	var delCond2 *model.TimeBeforeCond2
	fourteenDayBefore := 24 * 13
	fourteenDayBeforeStr := fmt.Sprintf("-%dh", fourteenDayBefore)
	d, _ := time.ParseDuration(fourteenDayBeforeStr)
	sec := time.Now().Add(d).Format("20060102")
	secInt, err := strconv.Atoi(sec)

	delCond2 = &model.TimeBeforeCond2{Uid: 1234,
		BeforeTime: model.DeleteTestLogCond{Day: int64(secInt)}}
	if uResult3, err = collection.DeleteMany(context.TODO(), delCond2); err != nil {
		fmt.Println(err.Error())
	}
	fmt.Println(uResult3.DeletedCount)
}
